    ...
    class EnsureThemekitInstalled < ShopifyCli::Task
      ...
      def call(ctx)
        ...
        begin
          ...
          ctx.puts(ctx.message('theme.ensure_themekit_installed.downloading', releases['version']))
          File.write(Themekit::THEMEKIT, Net::HTTP.get(URI.parse(release["url"])).force_encoding("UTF-8"))
          ctx.puts(ctx.message('theme.ensure_themekit_installed.verifying'))

          if Digest::MD5.file(Themekit::THEMEKIT) == release["digest"]
            FileUtils.chmod("+x", Themekit::THEMEKIT)
            ctx.puts(ctx.message('theme.ensure_themekit_installed.successful'))
          else
            ctx.puts(ctx.message('theme.ensure_themekit_installed.unsuccessful'))
            FileUtils.rm(Themekit::THEMEKIT)
          end
        rescue
          ctx.puts(ctx.message('theme.ensure_themekit_installed.failed'))
